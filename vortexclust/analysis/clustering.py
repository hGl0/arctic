...
from vortexclust.models.validation import check_unfitted_model
from .metrics import within_cluster_dispersion
from .sampling import generate_reference_HPPP
from vortexclust.core.utils import get_event_ranges
...
def gap_statistic(X: np.ndarray, k_max: int, n_replicates: int = 20, **kwargs) -> np.ndarray:
    ...
    Uses a Homogeneous Point Poisson Process to generate reference null distributions.
...
def split_displaced_seviour(data: np.ndarray,
                            ...
                            **kwargs):
    r"""
    Classify each day as 'split', 'displaced', or 'undisturbed' based on given thresholds:
      - Displaced: centroid latitude below 66°N (default) for 7 or more consecutive days
      - Split: aspect ratio above 2.4 (defult) for 7 or more consecutive days
    ...
    :type ar: float
    :param latcent: Latitude threshold, the latitude has to be equatorward of `latcent` for at least `days` days. Default is 66.
    :type latcent: float
    :param days: Number of days that a threshold has to be fulfilled. Default is 7.
    :type days: int
    :param kwargs:
        - `mark` which event should be marked: all, first, last

    :raises AttributeError: If `mark` is not one of 'all', 'first', 'last'
    :raises UserWarning: If data is numpy.ndarray. Expects columns to be in the order of `time_col`, `ar_col`, and `lat_col`.
    ...
    # DO NOT USE. Might lead to overflow error!
